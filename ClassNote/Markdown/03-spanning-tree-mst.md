...
long long prim(int n, const std::vector<std::vector<Adj>>& adj, int start = 0) {
    std::vector<bool> visited(n, false);
    using Item = std::tuple<int, int, int>;   // (w, u, v)
    std::priority_queue<Item, std::vector<Item>, std::greater<Item>> heap;

    visited[start] = true;
    for (const auto& e : adj[start]) heap.emplace(e.w, start, e.to);

    long long total = 0;
    int picked = 0;
    while (!heap.empty() && picked < n - 1) {
        ...
        for (const auto& e : adj[v])
            if (!visited[e.to]) heap.emplace(e.w, v, e.to);
    }
    return total;
}
```
...
> 둘 다 시간 복잡도는 동일 클래스 ($O(E \log V)$)이며, 어떤 자료구조가 친숙하냐로 고르세요.
...
신장 트리 중 **가장 큰 간선의 가중치**가 최소가 되는 트리. 사실: 모든 MST는 Bottleneck Spanning Tree이기도 합니다. (역은 거짓 — Bottleneck인 트리가 MST가 아닐 수 있음.)
...
public:
    bool validPath(int n, std::vector<std::vector<int>>& edges, int s, int t) {
        p.resize(n);
...
MST 알고리즘 자체는 아니지만, Union-Find가 *연결성*을 판단하는 핵심 도구임을 익히는 문제. Kruskal의 핵심 building block입니다.
</details>
...
public:
    int minCostConnectPoints(std::vector<std::vector<int>>& points) {
        int n = points.size();
        std::vector<bool> visited(n, false);
        using Item = std::pair<int,int>;  // (cost, point_idx)
        std::priority_queue<Item, std::vector<Item>, std::greater<Item>> heap;
        heap.emplace(0, 0);
        int total = 0, count = 0;
        while (!heap.empty() && count < n) {
            ...
            for (int v = 0; v < n; ++v) {
                if (visited[v]) continue;
                int d = std::abs(x1 - points[v][0]) + std::abs(y1 - points[v][1]);
                heap.emplace(d, v);
            }
        }
        return total;
...
완전 그래프(모든 점 쌍이 간선)이므로 dense → Prim이 자연스러움. 간선을 미리 만들 필요 없이, 매 정점에서 다른 모든 정점으로의 거리를 계산.
...
> 연결 성분의 개수를 세는 문제 (Union-Find 연습).
...
public:
    int swimInWater(std::vector<std::vector<int>>& grid) {
        ...
        using Item = std::tuple<int,int,int>;   // (t, r, c)
        std::priority_queue<Item, std::vector<Item>, std::greater<Item>> heap;
        ...
        while (!heap.empty()) {
            ...
            for (int k = 0; k < 4; ++k) {
                ...
                heap.emplace(std::max(t, grid[nr][nc]), nr, nc);
            }
        }
        return -1;
    }
...
"경로 위의 최대 가중치를 최소화" = bottleneck path 문제. Dijkstra와 거의 같지만 `+` 대신 `max`를 사용. MST의 cut property가 이 구조의 이론적 기반.
</details>
